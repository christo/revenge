...
const PRELOAD_DIR = path.resolve('./data/preload');
...
async function main() {
  ...
  console.log(`\nCollecting training data from ${PRELOAD_DIR}...`);
  ...
  const completeData = await collector.buildTrainingData(PRELOAD_DIR);
  ...
  // Example prediction
  const exampleFiles = findExampleFiles(PRELOAD_DIR);
