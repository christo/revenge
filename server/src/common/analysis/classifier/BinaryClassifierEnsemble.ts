...
export class BinaryClassifierEnsemble {
  ...
  async trainEnsemble(data: TrainingData): Promise<void> {
    ...
    // Convert features to normalized format
   ...
   */
  private normalizeFeatures(featureTuples: [string, number][]): number[] {
    const featureMap = new Map(featureTuples);
  ...
  private prepareTrainingData(data: TrainingData) {
    ...
    for (const fileId of fileIds) {
      ...
      const normalizedFeatures = this.normalizeFeatures(featureTuples);
      preparedFeatures.push(normalizedFeatures);
